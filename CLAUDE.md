...
- **Preact** + **Vite** for the web dashboard (builds to a single HTML file via vite-plugin-singlefile)
...
│       ├── dashboard.py         # pt dashboard open|dev (web UI)
...
dashboard/                           # Preact + Vite dashboard app
├── src/
│   ├── main.jsx                     # Entry: getData() → <App>
│   ├── App.jsx                      # Root: tabs, KPIs, all panels
│   ├── data.js                      # __PORTFOLIO_DATA__ (prod) or /api/data (dev)
│   ├── utils.js                     # fmt(), fmtPct(), COLORS
│   ├── hooks/useMarkdownExport.js   # generateMarkdown() + copyMarkdown()
│   ├── styles/index.css             # All CSS (Obsidian Terminal theme)
│   └── components/                  # ~15 components (overview/, holdings/, tax/, rebalancing/, analysis/)
├── index.html                       # Vite entry with __DATA_PLACEHOLDER__
├── vite.config.js                   # Preact preset + singlefile plugin
├── package.json
└── dist/index.html                  # Built single-file dashboard (committed)
...
- **Dashboard**: Preact + Vite app that builds to a single HTML file via `vite-plugin-singlefile`. Data is injected by Python via `__DATA_PLACEHOLDER__` replacement. `pt dashboard dev --vite` runs Vite HMR + Python API backend for development. Legacy `dashboard.html` still exists but is no longer the primary source.
...
pt dashboard dev [--vite] [--data-refresh]  # Dev server with live reload
...
```bash
make setup           # uv sync + dashboard npm install
uv run pt setup run  # configure tax profile on first run
uv run pt --help
```
...
```bash
make test                                # all 226 (or: uv run pytest)
uv run pytest tests/unit/test_tax/       # tax only
uv run pytest tests/integration/         # e2e only
```
...
make fix                                 # or: uv run ruff check --fix src/ tests/
make check                               # lint + tests in one command
```
